...
if (isset($_GET['event-id']) and !empty($_GET['event-id'])) {
    $event = \Eu\Rmmt\Event::getRepository()->find($_GET['event-id']);
}

if (!isset($_GET['expenditure-id']) or empty($_GET['event-id'])) {
    if (null !== $event) {
...
if (!isset($_POST['edit-expenditure'])) {
    ...
    $te->assign('events', \Eu\Rmmt\Event::getRepository()->findAll());
