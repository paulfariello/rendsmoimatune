...
class BdfMysqlDb implements BdfIDb {
  ...
  public function setType($type) {
    if($type != 'mysql') {
      trigger_error($type.' n\'est pas supporté par botte de foin',E_USER_ERROR);
    }
  }
  ...
  public function connect($server,$username,$password) { // Open a connection to a MySQL Server
    $this->link = mysql_connect($server,$username,$password) or BdfCore::getInstance()->logger->fatal("Connexion impossible à la base de donnée","Connection à la base de donnée");
    return $this->link;
  }
  ...
  public function escapeString() { // Escapes a string for use in a mysql_query
  ...
  public function fetchArray($res,$result_type=MYSQL_ASSOC) { // Fetch a result row as an associative array, a numeric array, or both
    if(!is_resource($res) OR get_resource_type($res) !== 'mysql result') {
      BdfCore::getInstance()->logger->error('Result argument is not a valid mysql result','SQL');
    }
  ...
  public function numRows($result) { // Get number of rows in result
    if(!is_resource($result) OR get_resource_type($result) != 'mysql result') {
      BdfCore::getInstance()->logger->error('Result argument is not a valid mysql result','SQL');
      return false;
    }
  ...
  public function result($result,$row,$field=0) { // Get result data
    $temp = mysql_result($result,$row,$field);
    if($temp === false)
