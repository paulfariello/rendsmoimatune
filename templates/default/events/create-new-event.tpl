...
{include file='inc/message.tpl'}
                	<form action="{makeUrl url='events/create-new-event.html'}" class="jNice" method="post">
                      <fieldset>
                          ...
                          <p><label for="start-date" class="float-left">{getText id='From'}</label><input type="text" id="start-date" class="text-medium date" name="start-date" {if isset($_POST)}value="{$_POST.{'start-date'}|htmlspecialchars}"{/if} />
                          <label for="end-date" class="float-left">{getText id='to'}</label><input type="text" id="end-date" class="text-medium date" name="end-date" {if isset($_POST)}value="{$_POST.{'end-date'}|htmlspecialchars}"{/if} /></p>
